...
import { Bell, MessageSquare, Target, Zap, Loader2, Sparkles, Check } from 'lucide-react';
...
export function NudgesPreview({ commitments, timeline }: Props) {
  ...
  useEffect(() => {
    async function fetchNudges() {
      try {
        const result = await generatePersonalizedNudges({ commitments, timeline });
        setNudges(result);
      } catch (error) {
  ...
  if (loading) {
    return (
      <Card className="glass-card p-12 text-center flex flex-col items-center justify-center min-h-[400px]">
        ...
        <h3 className="text-xl font-bold mb-2">Generating Personalized Nudges</h3>
        <p className="text-muted-foreground max-w-xs">Our AI is crafting custom reminders based on your specific goals...</p>
      </Card>
    );
  }
    ...
    <div className="space-y-6 animate-fade-in">
      <div className="text-center mb-8">
        <h2 className="text-3xl font-black mb-2 flex items-center justify-center gap-2">
          Your Smart Journey <Sparkles className="w-6 h-6 text-primary fill-primary/20" />
        </h2>
        <p className="text-muted-foreground">Stay on track with these AI-crafted notifications arriving on your schedule.</p>
      </div>
      ...
      <div className="space-y-4">
        {notificationCards.map((n, i) => (
          ...
          >
            <div className="absolute left-4 top-0 bottom-0 w-px bg-gradient-to-b from-border via-primary/20 to-border" />
            <div className="ml-12 bg-white rounded-3xl border border-border/50 shadow-lg hover:shadow-xl transition-all duration-300 p-6 flex gap-5 overflow-hidden">
      ...
      <Card className="glass-card mt-12 bg-primary text-white border-none text-center p-10 overflow-hidden relative">
        ...
        <CardHeader className="p-0 mb-6 relative z-10">
          <CardTitle className="text-3xl font-black">All Set!</CardTitle>
